...
for (i in start:end){
  data.curv <- data.vol[data.vol$PERIOD==i,]
  ...
  mv.data <- moutv$mdata
  data.prunedv <- rbind(data.prunedv, mv.data)
  ...
  mb.data <- moutb$mdata
  data.prunedb <- rbind(data.prunedb, mb.data)
}

data.prunedv$WEIGHT <- data.prunedv$CAP*moutv$weights #moutv$weights need to be replaced with the equivalent of ours; there is no column named weights
data.prunedb$WEIGHT <- data.prunedb$CAP*moutb$weights #moutb$weights need to be replaced with the equivalent of ours; there is no column named weights
...
#what is the role of this MatchBalance, is it supposed to be inside the for loop to be appended to data.pruned?
mb_v <- MatchBalance(TREATV ~ LOGCAP + LOGVOL + GROUP_COARSE_5 + GROUP_COARSE_8+ GROUP_COARSE_2 + GROUP_COARSE_4+ GROUP_COARSE_9+ GROUP_COARSE_6+ GROUP_COARSE_7+ GROUP_COARSE_3+ GROUP_COARSE_10,
             data = data.curv, match.out = moutv, nboots = 10000, print.level = 0)
mb_b <- MatchBalance(TREATB ~ LOGCAP + LOGVOL + GROUP_COARSE_5 + GROUP_COARSE_8+ GROUP_COARSE_2 + GROUP_COARSE_4+ GROUP_COARSE_9+ GROUP_COARSE_6+ GROUP_COARSE_7+ GROUP_COARSE_3+ GROUP_COARSE_10,
                     data = data.curb, match.out = moutb, nboots = 10000, print.level = 0)
mb_v$AMsmallest.p.value
mb_b$AMsmallest.p.value
summary(moutv)
summary(moutb)
moutv
moutb
